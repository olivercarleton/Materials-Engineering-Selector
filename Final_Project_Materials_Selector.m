...
fprintf('\n\n');
num=[];
num=input('How many metals do you wish to compare? You can enter up to 10: '); 
...
fprintf('\nGreat! You''ll need to enter some common properties. No need to worry about entering units.\n');
...
for k=1:num
    fprintf('Metal #%i: ',k)
    metals(k).metal=input('What is the name of the metal? ','s');
...
for k=1:num
    ...
    names(k)=string(metals(k).metal);
end


if num==1
    plot(EL,S); %Prints the stress/strain curve of the single metal.
    title('Mechanical Properties of',metals.metal);
    ylim([0 max(S)]);
